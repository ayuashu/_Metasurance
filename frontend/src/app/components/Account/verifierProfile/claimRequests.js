...
const ClaimRequests = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${HOST}/api/verifier/claim/get`, {
          method: 'GET',
          credentials: 'include',
        });
  ...
  const handleAccept = async (username, companyname, mappingid) => {
    ...
    console.log(username, companyname, mappingid, refundValue );
  };

  const handleSubmit = async () => {
    // if (Number(balance) < Number(refundValue)) {
    //   alert(`Insufficient balance, please purchase at least ${refundValue} tokens`);
    //   return;
    // }
    try {
      const response = await fetch(`${HOST}/api/verifier/claim/accept`, {
        method: 'POST',
        credentials: 'include',
        headers: {
        ...
        body: JSON.stringify({
          insuredname: selectedUsername,
          insurername: selectedCompanyname,
          mappingid: selectedMappingId,
          refund: String(refundValue),
        }),
      });
  ...
  const handleReject = async (username, mappingid) => {
    try {
      const response = await fetch(`${HOST}/api/verifier/claim/reject`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        console.error('Failed to reject the claim');
      }
    } catch (error) {
      console.error('Error rejecting the claim', error);
    }
  };
