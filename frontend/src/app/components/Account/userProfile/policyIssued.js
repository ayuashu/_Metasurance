    ...
    const fetchInsuranceCovers = async () => {
        try {
            ...
            if (response.ok) {
                ...
                if (data.reply && data.reply.policyCompanies && data.reply.policyCompanies.length > 0) {
                    ...
                    for (const company of policyCompanies) {
                        if (company.policies && company.policies.length > 0) {
                            for (const policy of company.policies) {
                                insuranceCoversMap.set(policy.policyid, policy.insurancecover);
                                console.log(insuranceCoversMap.get(policy.policyid))
                            }
                        }
                    }
    ...
    const fetchAssetName = async () => {
        try {
            ...
            if (response.ok) {
                ...
                if (data.reply && data.reply.assets && data.reply.assets.length > 0) {
    ...
    return (
        <div className="main-card-container">
            {policies && policies.length > 0 ? (
                    ...
                    return (
                        <div className="card-container" key={policyid}>
                            <div className="card" ref={cardRef}>
                                <span className="card-title" style={{ fontSize: '25px', fontWeight: 'bold' }}>
                                    Policy ID: {policyid}
                                </span>
                                    ...
                                    <table>
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td colSpan="3" style={{ paddingLeft: '20px' }}>
                                                    {isNaN(insuranceCoverData[policy.policyid]) ? 'Not available' : insuranceCoverData[policy.policyid] - premiumspaid}
                                                </td>
                                            </tr>
                                ...
                                <div>
                                    <button className="card-tag" onClick={() => handlePayPremium(mappingid)}>
                                        Pay Premium
                                    </button>
                                    <button className="card-tag" onClick={() => handleClaim(mappingid)} disabled={claimed}>
                                        Claim
                                    </button>
                                </div>
                            </div>
                ...
                <div className='grid grid-row-2 gap-10' style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                    <div className="text-white text-4xl p-5 pt-20" style={{ flex: '1 0 33%' }}>
                        <p style={{ fontSize: '60px', textAlign: 'center' }}>No policies found</p>
                        <br />
