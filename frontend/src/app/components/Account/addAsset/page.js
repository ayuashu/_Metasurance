...
const addAsset = () => {
    ...
    const handleLogin = async (e) => {
        ...
        if (username === "" || name === "" || assetName === "" || assetType === "" || userId === "" || assetValue === "" || age === "") {
            return alert("Complete all fields for registration")
        }
        const result = await fetch(`${HOST}/api/asset/register`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ username, name, assetName, assetType, userId, assetValue, age })
        })
        ...
        if (result.status === 200) {
            localStorage.setItem('asset', JSON.stringify({ username, name, assetName, assetType, userId, assetValue, age }))
            navigate('/components/Account/userProfile/')
        } else {
            alert(response.error)
        }
                            ...
                            <div className="flex flex-col md:flex-row">
                                ...
                                <div className="flex items-center justify-center  sm:p-12 md:w-1/2">
                                    <div className="w-full">
                                        ...
                                        <div>
                                            <label className="block mt-2 text-sm">UserId</label>
                                            <input type="text"
                                                className="w-full px-4 py-2 text-sm border rounded-md focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-600"
                                                value={userId}
                                        ...
                                        <div>
                                            <label className="block mt-2 text-sm">Value</label>
                                            <input type="number"
                                                className="w-full px-4 py-2 text-sm border rounded-md focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-600"
                                                value={assetValue}
